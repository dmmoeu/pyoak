...
from pyoak.legacy.error import (
...
@dataclass
class LegacyOtherNode(ASTNode):
    attr: str


@dataclass
class LegacyHiddenNonCompareAttrNode(ASTNode):
    attr: str
...
@dataclass
class LegacyParentNode(ASTNode):
    attr: str
    single_child: ASTNode
    child_list: tuple[ASTNode, ...]
    not_a_child_list: list[str]
    restricted_child: LegacyChildNode | LegacyOtherNode | None = None


@dataclass
class LegacySubParentNode(LegacyParentNode):
    extra_attr: str | None = None


@dataclass
class LegacyStaticFieldGettersTest(ASTNode):
    attr: str
    _hidden_attr: str
    child: ASTNode
...
def test_get_property_fields() -> None:
    ...
    assert _get_fname_set(LegacyStaticFieldGettersTest.get_property_fields()) == def_props
    assert _get_fname_set(
        LegacyStaticFieldGettersTest.get_property_fields(skip_hidden=False)
    ) == def_props | {
        "_hidden_attr",
    }
    assert _get_fname_set(
        LegacyStaticFieldGettersTest.get_property_fields(skip_non_compare=True)
    ) == def_props - {"non_compare_attr"}
    assert _get_fname_set(
        LegacyStaticFieldGettersTest.get_property_fields(skip_id=False)
    ) == def_props | {
        "id",
        "content_id",
    }
    assert _get_fname_set(
        LegacyStaticFieldGettersTest.get_property_fields(skip_origin=False)
    ) == def_props | {"origin"}
    assert _get_fname_set(
        LegacyStaticFieldGettersTest.get_property_fields(skip_original_id=False)
    ) == def_props | {
        "original_id",
    }
    assert _get_fname_set(
        LegacyStaticFieldGettersTest.get_property_fields(skip_id_collision_with=False)
    ) == def_props | {
        "id_collision_with",
    }
...
def test_get_child_fields() -> None:
    assert set(LegacyStaticFieldGettersTest.get_child_fields().keys()) == {
        "child",
        "optional_child",
        "child_list",
        "optional_child_list",
...
def test_id_handling() -> None:
    ...
    assert node == LegacyChildNode.get(node.id)
    assert ASTNode.get(node.id) is None
    assert node == ASTNode.get_any(node.id)

    # Implicitly set id
    node = LegacyChildNode("test", origin=origin)
    ...
    assert node.detach()
    new_node = LegacyChildNode("test", origin=origin)
    ...
    with pytest.raises(ASTNodeIDCollisionError) as excinfo:
        ...
        _ = LegacyChildNode("test", origin=origin, id=node.id, ensure_unique_id=True)

    assert excinfo.value.existing_node is new_node
    assert excinfo.value.new_node_class is LegacyChildNode

    with pytest.raises(ASTNodeIDCollisionError):
        # This should fail because the autogenerated id is already in use
        _ = LegacyChildNode("test", origin=origin, ensure_unique_id=True)

    # Test esnure unique id
    duplicate_node = LegacyChildNode("test", origin=origin)
    ...
    created_as_duplicate_node = LegacyChildNode("test", origin=origin, create_as_duplicate=True)
...
def test_detached_clones() -> None:
    ...
    for i in range(ch_count):
        ch_nodes.append(LegacyChildNode(str(i), origin=origin))

    parent = LegacyParentNode(
        attr="Test",
...
def test_parent_handling() -> None:
    ...
    assert cast(LegacyParentNode, excinfo.value.new_node).attr == "Test Second Parent"
...
def test_content_id() -> None:
    ...
    ch_nodes.append(LegacyChildNode("1", origin=different_origin))
...
def test_detach() -> None:
    ...
    assert all([LegacyChildNode.get(ch_nodes[i].id) is ch_nodes[i] for i in range(ch_count)])
...
def test_attach() -> None:
    ...
    existing_node = LegacyChildNode("Colliding Node", id=parent.id, origin=origin)
...
def test_replace() -> None:
    ...
    another_parent = LegacyParentNode(
        attr="Another Test",
        single_child=LegacyChildNode("Another Child", origin=origin),
        child_list=tuple(),
        not_a_child_list=[],
        origin=origin,
    )
    ...
    to_replace = LegacyHiddenNonCompareAttrNode("test", origin=origin)
    changes = dict(
    ...
    new_child = LegacyChildNode("New Child", origin=origin)
    ...
    assert LegacyParentNode.get(parent.id) is new_parent
...
def test_replace_with(caplog: pytest.LogCaptureFixture) -> None:
    ...
    old = LegacyChildNode("old", origin=origin)
    parent = parent.replace(restricted_child=old)
    another = LegacyParentNode(
        attr="Test2",
        single_child=LegacyChildNode("single", origin=origin),
        child_list=(),
        not_a_child_list=[],
        origin=origin,
    )
    ...
    assert exc_info.value.message.endswith(
        "because parent expects nodes of type: LegacyChildNode, LegacyOtherNode"
    )
    ...
    old = LegacyChildNode("attached old", origin=origin)
    new_child = ch_nodes[3]
    orig_id = new_child.id
    assert new_child.detached
    ...
    conflicting_child = LegacyChildNode("conflict", origin=origin)
    old = LegacyChildNode("old", origin=origin)
    attached_parent = LegacyParentNode(
        attr="Test2",
        single_child=old,
        child_list=(),
        not_a_child_list=[],
    ...
    new_child = LegacyChildNode(
        "new with conflict",
        id=conflicting_child.id,
        origin=origin,
        create_detached=True,
    )
    new = LegacyParentNode(
        attr="Test2",
    ...
    to_repace = LegacyChildNode("other", origin=origin)
    with_restricted = LegacyParentNode(
        attr="Test3",
        single_child=LegacyChildNode("some", origin=origin),
        child_list=(),
        not_a_child_list=[],
...
def test_duplicate() -> None:
    ...
    assert LegacyParentNode.get(dup_parent.original_id) is parent
...
def test_serialization_and_equality() -> None:
    ch_nodes: list[LegacyChildNode] = []
    ch_count = 10
    for i in range(ch_count):
        ch_nodes.append(LegacyChildNode(str(i), origin=origin))

    # Make sure we test with duplicate ids
    ch_nodes.append(LegacyChildNode(ch_nodes[-1].attr, origin=origin))
    ...
    deserialized = LegacyParentNode.from_dict(serialized_dict)
    ...
    assert parent == LegacyParentNode.from_dict(serialized_dict)


def test_collision_handling_at_deserialization() -> None:
    orig_node = LegacyChildNode("Test", origin=origin)
    dup_node = orig_node.duplicate()
    collided_node = LegacyChildNode("Test", origin=origin)
    ...
    orig_deserialized = LegacyChildNode.as_obj(orig_node.as_dict())
    dup_deserialized = LegacyChildNode.as_obj(dup_node.as_dict())
    collided_deserialized = LegacyChildNode.as_obj(collided_node.as_dict())
...
def test_ast_test_serialization_dialect() -> None:
    ...
    parent.detach()
    # Try to deserialize
    LegacyParentNode.from_dict(serialized_dialect_dict)
...
def test_to_msgpack() -> None:
    ...
    parent.detach()
    deserialized = LegacyParentNode.from_msgpck(serialized)

    assert parent == deserialized


def test_is_equal() -> None:
    one = LegacyChildNode("1", origin=origin)
    assert one.is_equal(one)

    two = LegacyChildNode("2", origin=origin)
    other_type_ast = LegacyOtherNode("1", origin=origin)
    ...
    other_origin_one = LegacyChildNode("1", origin=other_origin)
    assert one != other_origin_one
    assert one.is_equal(other_origin_one)

    collided_one = LegacyChildNode("1", origin=origin)
    ...
    created_as_duplicate_one = LegacyChildNode("1", origin=origin, create_as_duplicate=True)
    ...
    parent = LegacyParentNode(
        attr="Parent",
        single_child=LegacyChildNode("single_child", origin=origin),
        child_list=(
            LegacyChildNode("child_list1", origin=origin),
            LegacyChildNode("child_list2", origin=origin),
        ),
        not_a_child_list=[],
        origin=origin,
    )

    other_parent = LegacyParentNode(
        attr="Parent",
        single_child=LegacyChildNode("single_child", origin=other_origin),
        child_list=(
            LegacyChildNode("child_list1", origin=other_origin),
            LegacyChildNode("child_list2", origin=other_origin),
        ),
        not_a_child_list=[],
        origin=other_origin,
    )
    ...
    other_parent = LegacyParentNode(
        ...
        child_list=(
            LegacyChildNode("child_list1", origin=other_origin),
            LegacyChildNode("child_list2-changed", origin=other_origin),
        ),
        not_a_child_list=[],
        origin=other_origin,
    )
    ...
    hidone = LegacyHiddenNonCompareAttrNode(
        attr="1", _hidden_attr="one", non_compare_attr="one", origin=origin
    )
    hidtwo = LegacyHiddenNonCompareAttrNode(
        attr="1", _hidden_attr="two", non_compare_attr="two", origin=origin
    )
...
def test_to_content_dict() -> None:
    single_child = LegacyChildNode("single_child", origin=origin)
    child_list = LegacyChildNode("child_list1", origin=origin)
    parent = LegacyParentNode(
        attr="Parent",
...
@dataclass
class LegacyNested(ASTNode):
    attr: str


@dataclass
class LegacyMiddle(ASTNode):
    nested: LegacyNested | LegacyMiddle


@dataclass
class LegacyRoot(ASTNode):
    middle_tuple: tuple[LegacyMiddle, ...]


def test_xpath() -> None:
    n = LegacyNested("test", origin=origin)
    assert n.xpath is None

    assert n.calculate_xpath()
    assert n.xpath == "/@root[0]LegacyNested"

    m1 = LegacyMiddle(n, origin=origin)

    assert not n.calculate_xpath()
    assert m1.calculate_xpath()
    assert m1.xpath == "/@root[0]LegacyMiddle"
    assert n.xpath == "/@root[0]LegacyMiddle/@nested[0]LegacyNested"

    n2 = LegacyNested("test2", origin=origin)
    m2 = LegacyMiddle(n2, origin=origin)
    r = LegacyRoot((m1, m2), origin=origin)

    assert r.calculate_xpath()
    assert r.xpath == "/@root[0]LegacyRoot"
    assert m1.xpath == "/@root[0]LegacyRoot/@middle_tuple[0]LegacyMiddle"
    assert m2.xpath == "/@root[0]LegacyRoot/@middle_tuple[1]LegacyMiddle"
    assert n2.xpath == "/@root[0]LegacyRoot/@middle_tuple[1]LegacyMiddle/@nested[0]LegacyNested"

    r.detach()


def test_walkers() -> None:
    n1 = LegacyNested("test1", origin=origin)
    n2 = LegacyNested("test2", origin=origin)

    middle_nested1 = LegacyMiddle(n1, origin=origin)
    middle_nested2 = LegacyMiddle(n2, origin=origin)
    middle_middle = LegacyMiddle(middle_nested1, origin=origin)
    r = LegacyRoot((middle_nested2, middle_middle), origin=origin)
    ...
    # DFS, with filter
    assert list(r.dfs(filter=lambda n: not isinstance(n, LegacyNested) or n.attr != "test2")) == [
        r,
        middle_nested2,
        middle_middle,
        middle_nested1,
    ...
    assert list(
        r.dfs(bottom_up=True, filter=lambda n: not isinstance(n, LegacyMiddle) or n.nested != n1)
    ) == [
        n2,
        middle_nested2,
        n1,
    ...
    assert list(
        r.dfs(
            skip_self=True,
            filter=lambda n: isinstance(n, LegacyMiddle) and isinstance(n.nested, LegacyMiddle),
        )
    ) == [middle_middle]

    # DFS, with prune
    assert list(r.dfs(prune=lambda n: isinstance(n, LegacyMiddle))) == [
        r,
        middle_nested2,
        middle_middle,
    ]
    ...
    # BFS, with filter
    assert list(r.bfs(filter=lambda n: not isinstance(n, LegacyNested) or n.attr != "test2")) == [
        r,
        middle_nested2,
        middle_middle,
        middle_nested1,
    ...
    assert list(r.gather(LegacyMiddle, exact_type=True)) == [
        middle_nested2,
        middle_middle,
        middle_nested1,
    ]
    ...
    # Gather, with filter
    assert list(
        r.gather(LegacyNested, extra_filter=lambda n: cast(LegacyNested, n).attr != "test2")
    ) == [n1]

    r.detach()


class LegacyMiddleSubclass(LegacyMiddle):
    pass


def test_get_first_ancestor_of_type() -> None:
    n1 = LegacyNested("test1", origin=origin)
    m_m = LegacyMiddleSubclass(n1, origin=origin)
    m_r = LegacyMiddle(m_m, origin=origin)
    r = LegacyRoot((m_r,), origin=origin)

    assert n1.get_first_ancestor_of_type(LegacyMiddle) is m_m
    assert n1.get_first_ancestor_of_type(LegacyMiddle, exact_type=True) is m_r
    assert n1.get_first_ancestor_of_type(LegacyRoot) is r
    assert m_m.get_first_ancestor_of_type(LegacyMiddle) is m_r
    assert m_m.get_first_ancestor_of_type(LegacyMiddle, exact_type=True) is m_r
    assert m_m.get_first_ancestor_of_type(LegacyParentNode) is None
    assert m_m.get_first_ancestor_of_type(LegacyParentNode, exact_type=True) is None


def test_no_origin_serialization() -> None:
    node = LegacyChildNode("test", origin=NO_ORIGIN, create_detached=True)

    assert LegacyChildNode.from_json(node.to_json()) == node
