...
def _get_next_unique_id(id_: str) -> str:
    ...
    i = 1
    original_id = id_
    while AwareASTNode._nodes.get(id_) is not None:
...
def _set_xpath(node: AwareASTNode, parent_xpath: str) -> None:
...
@dataclass
class AwareASTNode(DataClassSerializeMixin):
    ...
    _nodes: t.ClassVar[
        weakref.WeakValueDictionary[str, AwareASTNode]
    ...
    """Mapping of field names to field instances & resolved types that are
    considered properties (i.e. not children). This mapping is determined
    statically at based only on field type annotations.

    Most methods check whether a field is a property or not by checking
    instances at runtime
    """

    _child_fields: t.ClassVar[t.Mapping[str, tuple[Field, ChildFieldTypeInfo]] | None] = None
    """"Mapping of field names to field instances & resolved types that are
    considered children. This mapping is determined statically at based only on
    field type annotations.

    Most methods check whether a field is a child or not by checking
    instances at runtime
    """
    ...
    """An ID of attached node whose Id was the same when this node was created
    (with ensure_unique_id option) or None if there was no collision.
    ...
    """The unique ID of this node. If not set in the constructor, it will be
    auto-generated based on the node's properties and origin.
    ...
    """When set to True, if a new node being created has non-unique ID an
    ASTNodeIDCollisionError exception is raised."""

    create_as_duplicate: InitVar[bool] = field(default=False, kw_only=True)
    """When set to True, if node collision registered, it is recorded as node
    duplication, not id collision. This is similar to getting the node from the
    registry and calling `duplicate` on it.

    This overrides `ensure_unique_id`.
    """

    create_detached: InitVar[bool] = field(default=False, kw_only=True)
    """When set to True, a node is not added to the registry upon creation.
    Nodes passed as children are not attached to the newly created node (will
    not have their parent set to the new node id) and id is not checked for
    collisions (id_collision_with, original_id are never set / ignored).
    ...
    def __post_init__(
        ...
        if not create_detached:
            if (existing_node := AwareASTNode._nodes.get(new_id)) is not None:
            ...
            if self.original_id is not None:
                ...
                if id_collision_with is not None:
                    ...
                    if id_collision_with == self.original_id or (
                        (collided := AwareASTNode._nodes.get(id_collision_with)) is not None
            ...
            if self.id_collision_with is not None:
                ...
                if id_collision_with is not None:
                    ...
                    if (
                        collided := AwareASTNode._nodes.get(id_collision_with)
    ...
    def _set_parent(self, parent: AwareASTNode, field: Field, index: int | None) -> None:
    ...
    def _set_content_id(self) -> None:
        ...
        Unlike automatic node ids, these do not include origin
        information, hidden & non-compare fields and hence by definition
        represent the content of the node.
        """
        hasher = hashlib.sha256()
        hasher.update(self.__class__.__name__.encode("utf-8"))
        for val, f in sorted(
    ...
    def _reset_content_id(self) -> None:
        """Recursively reset the content_id of this node and all its
        parents."""
        node: AwareASTNode | None = self
    ...
    def _attach_inner(
    ...
    ) -> tuple[AwareASTNode, AwareASTNode] | None:
        """Assigns parent to all child nodes, checking that they don't have a
        different parent that is also in the registry (aka attached node).
        ...
        if self.id in AwareASTNode._nodes:
            raise ASTNodeRegistryCollisionError(
                new_node=self,
                existing_node=AwareASTNode._nodes[self.id],
                operation=operation,
            )
        ...
        AwareASTNode._nodes[self.id] = self
    ...
    def _replace_child(
        self, old: AwareASTNode, field: Field, index: int | None, new: AwareASTNode | None
    ) -> None:
        """Replaces an `old` child node in `field` at index `index` with a
        `new` one.
        ...
        if index is not None:
            ...
            if new is not None:
                ...
                for c in t.cast(t.Iterable[AwareASTNode], orig_seq[index + 1 :]):
    ...
    def get(
        ...
        """
        ret = AwareASTNode._nodes.get(id, default)
        if ret is None:
            return None
    ...
    @classmethod
    def get_any(cls, id: str, default: AwareASTNode | None = None) -> AwareASTNode | None:
        ...
        """
        return AwareASTNode._nodes.get(id, default)

    def attach(self) -> None:
        """Attaches this node and all of it's detached children to the AST
        registry.
    ...
    def detach(self, *, only_self: bool = False) -> bool:
        """Removes this node and all of it's children (unless `only_self` is
        True) from the AST registry.
        ...
        AwareASTNode._nodes.pop(self.id)
    ...
    def replace_with(self: AwareASTNode, new: AwareASTNode | None) -> None:
        """Attempts to replace this node with a new one. If the new node has a
        different parent of if it's type does not match the type declaration of
        the parent node, the operation will raise ASTNodeReplaceWithError.
        ...
        if self.parent is not None:
            ...
            if new is not None:
                if not new.detached:
                    ...
                    AwareASTNode._nodes.pop(new.id, None)
                ...
                try:
                    ...
                    if new_was_attached:
                        AwareASTNode._nodes[new.id] = new
    ...
    def calculate_xpath(self) -> bool:
        ...
        This is a legacy method. The use of match.xpath module is
        preferred.
        """
        if not self.is_attached_root:
    ...
    @property
    def children(self) -> t.List[AwareASTNode]:
    ...
    @property
    def parent(self) -> AwareASTNode | None:
        """Returns the parent node of this node or None if it is the root
        node."""
        ...
        if parent_id is None:
            ...
            return AwareASTNode.get_any(parent_id)

    @property
    def parent_field(self) -> Field | None:
        """Returns the field that this node is assigned to in the parent
        node."""
        # Since we are dynamically assigning the parent fields
        # we need to use getattr to make typing happy
    ...
    def parent_index(self) -> int | None:
        """Returns the index of this node in the parent node's container field
        (list or tuple)."""
        # Since we are dynamically assigning the parent fields
        # we need to use getattr to make typing happy
    ...
    def detached(self) -> bool:
        ...
        return AwareASTNode._nodes.get(self.id) is not self
    ...
    def get_depth(
        self, relative_to: AwareASTNode | None = None, check_ancestor: bool = True
    ) -> int:
        """Returns the depth of this node in the tree either up to root or up
        to `relative_to` node (if it is the ancestor at all).
    ...
    def ancestors(self) -> t.Iterator[AwareASTNode]:
    ...
    def get_first_ancestor_of_type(
        ...
        *,
        ...
        """Returns the first ancestor of this node that is an instance of
    ...
    def is_ancestor(self, node: AwareASTNode) -> bool:
    ...
    def is_equal(self, other: t.Any) -> bool:
        ...
        ignores the origin, id, parent, etc.
        """
    ...
    def _is_field_child(self, field: Field) -> bool:
        ...
        if isinstance(o, AwareASTNode):
            return True
        else:
            for item in self._ensure_iterable(o):
                if isinstance(item, AwareASTNode):
                    return True
    ...
    def _iter_child_fields(
        self,
    ) -> t.Iterable[tuple[AwareASTNode | t.Sequence[AwareASTNode] | None, Field]]:
        for f in fields(self):
    ...
    def dfs(
        self,
        prune: t.Callable[[AwareASTNode], bool] | None = None,
        filter: t.Callable[[AwareASTNode], bool] | None = None,
        bottom_up: bool = False,
        skip_self: bool = False,
    ) -> Generator[AwareASTNode, None, None]:
        ...
        """
        build_queue: t.Deque[AwareASTNode] = deque([self])
        yield_queue: t.Deque[AwareASTNode] = deque()
    ...
    def bfs(
        ...
        """
        queue: t.Deque[AwareASTNode] = deque([self])
    ...
    def gather(
        ...
        *,
        exact_type: bool = False,
        extra_filter: t.Callable[[AwareASTNode], bool] | None = None,
        prune: t.Callable[[AwareASTNode], bool] | None = None,
        skip_self: bool = False,
        ...
        if not exact_type:
            ...
            def filter_fn(obj: AwareASTNode) -> bool:
    ...
    def get_child_nodes(self) -> t.Iterable[AwareASTNode]:
        ...
        for f in fields(self):
            ...
            for o in objects:
                assert isinstance(o, AwareASTNode)
                yield o

    def get_child_nodes_with_field(
        self,
    ) -> t.Iterable[tuple[AwareASTNode, Field, int | None]]:
        """Returns a generator object which yields all child nodes with their
        corresponding field and index (for lists and tuples)."""
        for f in fields(self):
            # Skip non-child fields
            if not self._is_field_child(f):
                continue
...
ASTNodeType = t.TypeVar("ASTNodeType", bound=AwareASTNode)
...
class ASTVisitor(t.Generic[VisitorReturnType]):
    ...
    def generic_visit(self, node: AwareASTNode) -> VisitorReturnType:
        raise NotImplementedError

    def visit(self, node: AwareASTNode) -> VisitorReturnType:
...
class ASTTransformVisitor(ASTVisitor[AwareASTNode | None]):
    ...
    def _transform_children(
        self, node: AwareASTNode
    ) -> t.Mapping[str, AwareASTNode | None | list[AwareASTNode] | tuple[AwareASTNode, ...]]:
        ...
        """
        changes: dict[str, AwareASTNode | None | list[AwareASTNode] | tuple[AwareASTNode, ...]] = {}
        ...
        for fname in field_names_with_changes:
            ...
            if type_info.sequence_type is not None:
                changes[fname] = type_info.sequence_type(t.cast(list[AwareASTNode], changes[fname]))

        # Return the changes
        return changes

    def generic_visit(self, node: AwareASTNode) -> AwareASTNode | None:
    ...
    def visit(self, node: AwareASTNode) -> AwareASTNode | None:
        """Overrides the default visit method to ensure that the node is
        detached before transformation.
    ...
    def transform(self, node: AwareASTNode) -> AwareASTNode | None:
        """Transforms a given node and returns the transformed node or None if
        ...
        Raises:
            ASTTransformError: If the transformation fails with the original exception
                as context.
        """
        orig_node: AwareASTNode | None = None
        ...
        transformed: AwareASTNode | None = None
        try:
...
class ASTTransformer(ABC):
    ...
    def prune(self, node: AwareASTNode) -> bool:
    ...
    def filter(self, node: AwareASTNode) -> bool:
    ...
    def execute(self, node: AwareASTNode) -> AwareASTNode | None:
        """Executes the `transform` function defined on this class against all
        nodes in subtree rooted in `node`, applying filter and pruning function
        as defined on this class.
